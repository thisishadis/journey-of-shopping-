...
const ProductList = () => {
    ...
    const { isPending, error, data: productss } = useQuery({
        queryKey: ['myData'],
        queryFn: async () => {
           try {
    ...
    // flter based on free delivery
    const {isPending:delivery, error:deliveryError, data: freeDeliveryProducts} = useQuery({
    ...
    const {isPending:bestSeller, error:bestSellerError, data: bestSellerProducts} = useQuery({
        queryKey: ['freeDeliveryProducts'],
        queryFn: () =>
            axios.get(
